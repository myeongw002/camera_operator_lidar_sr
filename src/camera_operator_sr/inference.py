...
@torch.no_grad()
def fuse_observed_ranges(output, batch: dict, exact_tolerance: float = 1e-5):
    """Preserve real 16-beam observations in exported 64-row predictions."""
    prediction = output.predicted_range.clone()
...
def fuse_observed_rows(output, batch: dict, exact_tolerance: float = 1e-5):
    """Legacy range/return fusion wrapper."""
    prediction = fuse_observed_ranges(output, batch, exact_tolerance)
    probability = output.return_probability.clone()
    input_elevation, target_elevation = batch["lidar"]["elevation"], batch["target"]["elevation"]
    if input_elevation.ndim == 1: input_elevation = input_elevation[None]
    if target_elevation.ndim == 1: target_elevation = target_elevation[None]
    for batch_index in range(prediction.shape[0]):
        target_index = (target_elevation[batch_index, :, None] - input_elevation[batch_index, None, :]).abs().argmin(dim=0)
        exact = (target_elevation[batch_index, target_index] - input_elevation[batch_index]).abs().le(exact_tolerance)
        probability[batch_index, :, target_index[exact]] = batch["lidar"]["valid"][batch_index, :, torch.arange(input_elevation.shape[1], device=prediction.device)[exact]]
