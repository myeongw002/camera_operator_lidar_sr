...
def test_teacher_exact_resume_matches_continuous_training(tmp_path):
    make_dataset(tmp_path); run(["scripts/train_student.py", *common(tmp_path, "student"), "--epochs", "1"])
    baseline = checkpoint(tmp_path, "student")
    common_teacher = ["--baseline-checkpoint", str(baseline), "--depth-mode", "correct"]
    run(["scripts/train_teacher.py", *common_teacher, *common(tmp_path, "continuous"), "--epochs", "2"])
    run(["scripts/train_teacher.py", *common_teacher, *common(tmp_path, "resumed"), "--epochs", "1"])
    run(["scripts/train_teacher.py", *common_teacher, *common(tmp_path, "resumed"), "--epochs", "2", "--resume"])
